...
@njit(cache=True)
def intracell_energies_staggered(spins, J):
    J2 = J[2] if len(J) > 2 else 0.0
...
@njit(cache=True)
def log_Rpp_Rpm_staggered(r, J):
    D = len(J) - 1

    # Intracell energies
    E_plus  = intracell_energies_staggered(plus_configs,  J)
    E_minus = intracell_energies_staggered(minus_configs, J)
